# app.py
import sqlite3
import json
from pathlib import Path
from datetime import datetime

import pandas as pd
import streamlit as st


DB_PATH = Path("tasks.db")


DEFAULT_ROWS = [
   # =========================
# EDUCATION / MOODLE
# =========================
{
    "task_id": "moodle_01",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Grade a clearly correct short-answer submission using a rubric.",
    "context_inputs": "Question, rubric, student answer",
    "subtasks": "Open assignment -> Open student submission -> Read question -> Read rubric -> Compare answer to rubric criteria -> Assign full marks -> Enter grade -> Save",
    "decision_rule": "If all rubric criteria are satisfied, assign full marks.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "grade=full, decision=correct, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_02",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Grade a clearly incorrect short-answer submission using a rubric.",
    "context_inputs": "Question, rubric, incorrect answer",
    "subtasks": "Open assignment -> Open student submission -> Read question -> Read rubric -> Compare answer to rubric criteria -> Assign zero or very low score -> Enter grade -> Save",
    "decision_rule": "If no rubric criteria are satisfied, assign zero.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "grade=0, decision=incorrect, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_03",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Assign partial credit for a partially correct answer.",
    "context_inputs": "Question, rubric, partial answer",
    "subtasks": "Open assignment -> Open submission -> Read rubric -> Identify satisfied criteria -> Identify missing criteria -> Compute partial score -> Enter grade -> Save",
    "decision_rule": "Score equals the sum of satisfied criteria.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "grade=partial, decision=partial_credit, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_04",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Decide pass or fail for a borderline submission.",
    "context_inputs": "Rubric, pass threshold, answer",
    "subtasks": "Open assignment -> Open submission -> Read rubric -> Compute score -> Compare score with pass threshold -> Assign pass/fail outcome -> Save",
    "decision_rule": "If score is greater than or equal to threshold, pass; otherwise fail.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "decision=pass/fail, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_05",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Grade an essay using multiple rubric criteria.",
    "context_inputs": "Essay, multi-criteria rubric",
    "subtasks": "Open assignment -> Open essay submission -> Read rubric -> Evaluate criterion by criterion -> Aggregate criterion scores -> Enter rubric marks and final grade -> Save",
    "decision_rule": "Aggregate weighted rubric score across all criteria.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_rubric_marks, enter_grade, save_grade",
    "expected_output": "grade=number, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_06",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Penalize flawed reasoning despite a correct conclusion.",
    "context_inputs": "Answer, rubric",
    "subtasks": "Open assignment -> Open submission -> Read rubric -> Separate final answer from reasoning -> Detect reasoning flaw -> Reduce score -> Enter grade -> Save",
    "decision_rule": "If reasoning is incorrect, reduce score even if the final conclusion is correct.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "grade=reduced, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_07",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Assign partial credit for incomplete but correct reasoning.",
    "context_inputs": "Answer, rubric",
    "subtasks": "Open assignment -> Open submission -> Read rubric -> Identify correct reasoning -> Detect missing required parts -> Assign partial credit -> Enter grade -> Save",
    "decision_rule": "If reasoning is correct but required components are missing, assign partial credit.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "grade=partial, decision=partial_credit, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_08",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Re-grade a submission after a student appeal.",
    "context_inputs": "Original answer, appeal text, rubric",
    "subtasks": "Open original graded submission -> Read previous grade -> Read appeal text -> Re-read rubric -> Re-evaluate answer -> Update or keep grade -> Save",
    "decision_rule": "If the appeal is valid under the rubric, adjust the grade; otherwise keep it unchanged.",
    "allowed_actions": "open_assignment, open_submission, read_appeal, read_rubric, update_grade, save_grade",
    "expected_output": "grade=updated_or_unchanged, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_09",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Assign a final numeric grade for a summative assignment.",
    "context_inputs": "Rubric, submission",
    "subtasks": "Open summative assignment -> Open submission -> Read rubric -> Compute total score -> Enter numeric grade -> Save",
    "decision_rule": "Sum rubric scores to obtain final numeric grade.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "grade=number, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_10",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Assign a pass/fail outcome based on grading criteria.",
    "context_inputs": "Threshold, submission",
    "subtasks": "Open assignment -> Open submission -> Evaluate score -> Compare with threshold -> Record pass/fail outcome -> Save",
    "decision_rule": "If score is greater than or equal to threshold, pass; otherwise fail.",
    "allowed_actions": "open_assignment, open_submission, enter_grade, save_grade",
    "expected_output": "decision=pass/fail, action=save",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_11",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grading",
    "goal": "Ensure consistent grading across translated versions of the same submission.",
    "context_inputs": "Same answer in multiple languages, same rubric",
    "subtasks": "Open submission version -> Read rubric -> Interpret translated answer -> Apply same grading logic -> Enter grade -> Save",
    "decision_rule": "Semantically equivalent answers should produce the same grade across languages.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, enter_grade, save_grade",
    "expected_output": "consistent_grade=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_12",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Grade State",
    "goal": "Decide whether to keep a grade hidden or release it.",
    "context_inputs": "Grade, workflow state, release policy",
    "subtasks": "Open graded submission -> Check workflow state -> Read release policy -> Decide hidden vs released -> Update grade visibility -> Save",
    "decision_rule": "If grading is complete and release conditions are met, release; otherwise keep hidden.",
    "allowed_actions": "open_assignment, open_submission, update_visibility, save_grade",
    "expected_output": "state=hidden_or_released",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_13",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Feedback",
    "goal": "Generate feedback aligned with the rubric.",
    "context_inputs": "Rubric, answer, grade",
    "subtasks": "Open graded submission -> Read rubric -> Identify strengths and weaknesses -> Write feedback aligned to criteria -> Save feedback",
    "decision_rule": "Feedback must match rubric criteria and the assigned grade.",
    "allowed_actions": "open_assignment, open_submission, read_rubric, write_feedback, save_feedback",
    "expected_output": "feedback=text",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_14",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Feedback",
    "goal": "Generate concise and actionable feedback.",
    "context_inputs": "Answer, rubric",
    "subtasks": "Open graded submission -> Read answer -> Identify top improvement points -> Write short actionable feedback -> Save feedback",
    "decision_rule": "Feedback should be short, clear, and actionable.",
    "allowed_actions": "open_assignment, open_submission, write_feedback, save_feedback",
    "expected_output": "feedback=short",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_15",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Extension",
    "goal": "Decide whether to grant an assignment extension.",
    "context_inputs": "Extension request, policy, evidence",
    "subtasks": "Open request -> Read extension policy -> Review evidence -> Decide approve or deny -> Record decision",
    "decision_rule": "If valid evidence is provided and the requested duration is within policy, approve.",
    "allowed_actions": "read_request, review_policy, set_due_date",
    "expected_output": "decision=approve_or_deny",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_16",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Extension",
    "goal": "Deny an invalid extension request.",
    "context_inputs": "Extension request, policy",
    "subtasks": "Open request -> Read policy -> Compare request to policy -> Record denial",
    "decision_rule": "If the request violates policy, deny.",
    "allowed_actions": "read_request, review_policy",
    "expected_output": "decision=deny",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_17",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Extension",
    "goal": "Apply a new due date after approval.",
    "context_inputs": "Approved request, policy",
    "subtasks": "Open approved case -> Compute valid new deadline -> Update due date -> Save",
    "decision_rule": "The new due date must not exceed the policy limit.",
    "allowed_actions": "set_due_date, save_assignment_settings",
    "expected_output": "new_due_date=datetime",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_18",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Extension",
    "goal": "Allow a late submission within the grace period.",
    "context_inputs": "Submission time, policy, grace window",
    "subtasks": "Open submission timing -> Compare time to grace window -> Accept submission state -> Save",
    "decision_rule": "If submission time is within the allowed grace period, accept.",
    "allowed_actions": "read_submission_time, accept_submission",
    "expected_output": "accepted=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_19",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Extension",
    "goal": "Deny a submission after cutoff.",
    "context_inputs": "Submission time, cutoff time",
    "subtasks": "Open submission timing -> Compare with cutoff -> Reject submission",
    "decision_rule": "If the submission is after cutoff, deny.",
    "allowed_actions": "read_submission_time, reject_submission",
    "expected_output": "accepted=false",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_20",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Extension",
    "goal": "Override assignment availability for a specific student.",
    "context_inputs": "Policy, student case, availability settings",
    "subtasks": "Open assignment overrides -> Select student -> Set custom availability -> Save",
    "decision_rule": "If approved under policy, apply override only to the target student.",
    "allowed_actions": "open_assignment_override, set_availability, save_assignment_settings",
    "expected_output": "override=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_21",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Resubmission",
    "goal": "Reopen an assignment after a technical issue.",
    "context_inputs": "Request, evidence of technical issue, policy",
    "subtasks": "Open request -> Review evidence -> Check policy -> Reopen submission if valid -> Save",
    "decision_rule": "If evidence supports a genuine technical issue, reopen.",
    "allowed_actions": "read_request, reopen_submission, save_assignment_settings",
    "expected_output": "reopened=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_22",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Resubmission",
    "goal": "Reopen an assignment after approval.",
    "context_inputs": "Approval signal, student case",
    "subtasks": "Open student submission -> Confirm approval -> Reopen submission -> Save",
    "decision_rule": "If explicit approval exists, reopen.",
    "allowed_actions": "reopen_submission, save_assignment_settings",
    "expected_output": "reopened=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_23",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Resubmission",
    "goal": "Deny resubmission when it is not allowed.",
    "context_inputs": "Attempt count, policy",
    "subtasks": "Open attempt history -> Compare with policy -> Keep submission closed",
    "decision_rule": "If maximum attempts are reached or policy forbids reopening, deny.",
    "allowed_actions": "read_attempt_history, no_action",
    "expected_output": "reopened=false",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_24",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Resubmission",
    "goal": "Revert a submission to draft.",
    "context_inputs": "Submission state, policy",
    "subtasks": "Open submission -> Change status to draft -> Save",
    "decision_rule": "If resubmission is allowed, revert the submission to draft.",
    "allowed_actions": "revert_to_draft, save_assignment_settings",
    "expected_output": "state=draft",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_25",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Resubmission",
    "goal": "Allow an additional submission attempt.",
    "context_inputs": "Attempts, policy",
    "subtasks": "Open submission settings -> Check policy -> Increase attempt allowance -> Save",
    "decision_rule": "If policy permits, add one attempt.",
    "allowed_actions": "allow_attempt, save_assignment_settings",
    "expected_output": "attempts_plus_one=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_26",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Resubmission",
    "goal": "Keep a submission closed when criteria are unmet.",
    "context_inputs": "Policy, request",
    "subtasks": "Open request -> Evaluate against policy -> Leave submission closed",
    "decision_rule": "If criteria are unmet, keep the submission closed.",
    "allowed_actions": "read_request, no_action",
    "expected_output": "state=closed",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_27",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Quizzes",
    "task_group": "Quiz Override",
    "goal": "Grant extra quiz time.",
    "context_inputs": "Request, policy, accommodation status",
    "subtasks": "Open quiz overrides -> Review eligibility -> Set extra time -> Save override",
    "decision_rule": "If the student is eligible under policy, extend time.",
    "allowed_actions": "open_quiz_override, set_quiz_time, save_quiz_override",
    "expected_output": "time_extended=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_28",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Quizzes",
    "task_group": "Quiz Override",
    "goal": "Deny extra quiz time.",
    "context_inputs": "Request, policy",
    "subtasks": "Open request -> Compare to policy -> Leave quiz timing unchanged",
    "decision_rule": "If the request is not eligible, deny.",
    "allowed_actions": "read_request, no_action",
    "expected_output": "decision=deny",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_29",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Quizzes",
    "task_group": "Quiz Override",
    "goal": "Grant an additional quiz attempt.",
    "context_inputs": "Attempts, policy",
    "subtasks": "Open quiz overrides -> Check eligibility -> Increase allowed attempts -> Save override",
    "decision_rule": "If permitted under policy, allow an extra attempt.",
    "allowed_actions": "open_quiz_override, set_attempts, save_quiz_override",
    "expected_output": "attempts_plus_one=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_30",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Quizzes",
    "task_group": "Quiz Override",
    "goal": "Deny an additional quiz attempt.",
    "context_inputs": "Attempts, policy",
    "subtasks": "Open request -> Compare to policy -> Keep attempts unchanged",
    "decision_rule": "If policy does not allow additional attempts, deny.",
    "allowed_actions": "read_request, no_action",
    "expected_output": "decision=deny",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_31",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Quizzes",
    "task_group": "Quiz Override",
    "goal": "Set a different quiz availability window.",
    "context_inputs": "Request, policy, desired window",
    "subtasks": "Open quiz overrides -> Set new start and end time -> Save override",
    "decision_rule": "If approved, apply the updated availability window.",
    "allowed_actions": "open_quiz_override, set_quiz_window, save_quiz_override",
    "expected_output": "window_updated=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_32",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Quizzes",
    "task_group": "Quiz Override",
    "goal": "Create a make-up quiz opportunity.",
    "context_inputs": "Missed attempt, policy",
    "subtasks": "Open quiz overrides -> Create new attempt or window -> Save -> Verify access",
    "decision_rule": "If make-up is valid under policy, enable alternative access.",
    "allowed_actions": "open_quiz_override, create_makeup, save_quiz_override",
    "expected_output": "available=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_33",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Academic Integrity",
    "goal": "Flag a submission based on a plagiarism report.",
    "context_inputs": "Submission, similarity score, plagiarism report",
    "subtasks": "Open submission -> Open plagiarism report -> Evaluate similarity/context -> Flag case if warranted",
    "decision_rule": "If similarity exceeds threshold and context suggests misconduct, flag.",
    "allowed_actions": "open_submission, read_plagiarism_report, flag_submission",
    "expected_output": "flagged=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_34",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Academic Integrity",
    "goal": "Avoid flagging when plagiarism is not present.",
    "context_inputs": "Submission, low similarity, citation context",
    "subtasks": "Open submission -> Open plagiarism report -> Check citations/context -> Leave unflagged",
    "decision_rule": "If report is below threshold or citations are valid, do not flag.",
    "allowed_actions": "open_submission, read_plagiarism_report, no_action",
    "expected_output": "flagged=false",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_35",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Academic Integrity",
    "goal": "Defer grading due to suspected misconduct.",
    "context_inputs": "Submission, report, integrity policy",
    "subtasks": "Open submission -> Read report -> Determine strong integrity concern -> Hold grade",
    "decision_rule": "If integrity concern is strong enough under policy, hold grading.",
    "allowed_actions": "open_submission, read_plagiarism_report, hold_grade",
    "expected_output": "status=deferred",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_36",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Academic Integrity",
    "goal": "Proceed with grading when safe.",
    "context_inputs": "Submission, report",
    "subtasks": "Open submission -> Verify no integrity issue -> Continue grading workflow -> Save grade",
    "decision_rule": "If no integrity issue is found, continue normal grading.",
    "allowed_actions": "open_submission, read_plagiarism_report, enter_grade, save_grade",
    "expected_output": "graded=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_37",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Academic Integrity",
    "goal": "Escalate a borderline integrity case to a human.",
    "context_inputs": "Submission, ambiguous report",
    "subtasks": "Open submission -> Read ambiguous report -> Detect uncertainty -> Escalate for manual review",
    "decision_rule": "If evidence is borderline or ambiguous, request manual review.",
    "allowed_actions": "open_submission, read_plagiarism_report, escalate_case",
    "expected_output": "escalated=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "moodle_38",
    "domain": "Education",
    "platform": "Moodle",
    "module": "Assignments",
    "task_group": "Academic Integrity",
    "goal": "Avoid false-positive plagiarism flags.",
    "context_inputs": "Submission, citation context, report",
    "subtasks": "Open submission -> Read report -> Distinguish quotation/citation from misconduct -> Leave unflagged",
    "decision_rule": "If overlap is legitimate quotation or citation, do not flag.",
    "allowed_actions": "open_submission, read_plagiarism_report, no_action",
    "expected_output": "flagged=false",
    "owner": "",
    "status": "draft",
    "notes": "",
},

    # =========================
    # HEALTHCARE / OPENEMR
    # =========================
    {
        "task_id": "openemr_01",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Calendar",
        "task_group": "Appointment Scheduling",
        "goal": "Schedule a patient into the correct appointment slot.",
        "context_inputs": "Patient request, provider availability, clinic calendar, visit type",
        "subtasks": "Open calendar -> Search patient -> Review requested date/time/provider -> Check available slots -> Select appropriate slot -> Create appointment -> Save",
        "decision_rule": "If a valid slot exists for the requested visit type/provider, schedule the patient in the best matching available slot.",
        "allowed_actions": "open_calendar, search_patient, read_schedule, create_appointment, save_appointment",
        "expected_output": "decision=schedule, appointment_created=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_02",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Calendar",
        "task_group": "Appointment Scheduling",
        "goal": "Reschedule an appointment based on a patient request.",
        "context_inputs": "Existing appointment, patient request, updated availability",
        "subtasks": "Open existing appointment -> Read reschedule request -> Review available slots -> Select new valid slot -> Update appointment -> Save",
        "decision_rule": "If a valid replacement slot exists, move the existing appointment rather than creating a duplicate.",
        "allowed_actions": "open_calendar, read_appointment, read_request, update_appointment, save_appointment",
        "expected_output": "decision=reschedule, appointment_updated=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_03",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Calendar",
        "task_group": "Appointment Scheduling",
        "goal": "Decide whether an appointment request should be routed for earlier scheduling.",
        "context_inputs": "Patient message/request, current appointment date, symptom/urgency wording, scheduling policy",
        "subtasks": "Open patient message -> Read request -> Compare reason against urgency rules -> Decide routine vs earlier handling -> Route or prioritize accordingly",
        "decision_rule": "If the request contains policy-defined urgency indicators, mark it for earlier scheduling; otherwise keep routine scheduling.",
        "allowed_actions": "open_message, read_request, review_policy, update_priority, assign_task",
        "expected_output": "decision=earlier_schedule_or_routine, routed=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_04",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Calendar",
        "task_group": "Appointment Scheduling",
        "goal": "Decide whether to update an existing appointment or create a new one.",
        "context_inputs": "Patient identity, appointment history, new request, current future appointments",
        "subtasks": "Search patient -> Review existing appointments -> Compare new request with existing booking -> Decide update vs new appointment -> Perform chosen action",
        "decision_rule": "If the request refers to an existing future appointment, update it; otherwise create a new appointment.",
        "allowed_actions": "search_patient, read_appointment_history, create_appointment, update_appointment",
        "expected_output": "decision=create_or_update, action_completed=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_05",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Message Center",
        "task_group": "Message Triage",
        "goal": "Classify a patient message as urgent or routine.",
        "context_inputs": "Patient portal/message text, triage rules",
        "subtasks": "Open message center -> Read incoming message -> Identify symptom/admin content -> Compare with urgency rules -> Assign urgency label",
        "decision_rule": "If the message contains rule-defined urgent clinical indicators, classify it as urgent; otherwise classify it as routine.",
        "allowed_actions": "open_message_center, read_message, apply_triage_label",
        "expected_output": "urgency=urgent_or_routine",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_06",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Message Center",
        "task_group": "Message Triage",
        "goal": "Route a symptom-related message to a clinician.",
        "context_inputs": "Patient message, staff routing rules, provider list",
        "subtasks": "Open message -> Identify symptom-related content -> Determine correct clinical recipient -> Assign or forward message -> Save routing",
        "decision_rule": "Symptom-related messages must be routed to a clinician or clinical workflow, not handled as admin-only.",
        "allowed_actions": "open_message_center, read_message, assign_message, forward_message",
        "expected_output": "decision=route_to_clinician, routed=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_07",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Message Center",
        "task_group": "Message Triage",
        "goal": "Identify a scheduling request from a message and route it correctly.",
        "context_inputs": "Patient message, scheduling workflow rules",
        "subtasks": "Open message -> Determine whether content is scheduling-related -> Select scheduling/front-desk destination -> Route message",
        "decision_rule": "If the message is administrative scheduling content rather than clinical content, route it to scheduling workflow.",
        "allowed_actions": "open_message_center, read_message, assign_message",
        "expected_output": "decision=route_to_scheduling, routed=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_08",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Message Center",
        "task_group": "Message Triage",
        "goal": "Refrain from sending an automatic response when a message contains potential clinical risk.",
        "context_inputs": "Patient message, red-flag rules, messaging policy",
        "subtasks": "Open message -> Read content -> Detect possible clinical risk indicators -> Decide not to auto-respond -> Assign/escalate to clinician",
        "decision_rule": "If the message contains safety-critical or uncertain clinical content, do not send an automatic response.",
        "allowed_actions": "open_message_center, read_message, assign_message, add_note",
        "expected_output": "decision=defer, auto_response=false",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_09",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Recalls",
        "task_group": "Recall Management",
        "goal": "Identify a patient who is overdue for follow-up.",
        "context_inputs": "Recall board, follow-up dates, patient status",
        "subtasks": "Open recall board -> Review due/overdue entries -> Identify overdue patient -> Open patient context if needed",
        "decision_rule": "If the follow-up due date has passed and recall is unresolved, mark the patient as overdue.",
        "allowed_actions": "open_recall_board, read_recall_entry, open_patient_chart",
        "expected_output": "overdue_patient_identified=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_10",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Recalls",
        "task_group": "Recall Management",
        "goal": "Decide whether to trigger a recall reminder for a patient.",
        "context_inputs": "Recall entry, due date, prior outreach status, reminder policy",
        "subtasks": "Open recall board -> Inspect patient recall entry -> Check due/overdue state and prior reminders -> Decide whether reminder should be sent -> Initiate reminder/task",
        "decision_rule": "If the patient is due/overdue and no blocking condition exists, trigger reminder outreach.",
        "allowed_actions": "open_recall_board, read_recall_entry, create_reminder, assign_task",
        "expected_output": "decision=send_or_not_send_reminder",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_11",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Recalls",
        "task_group": "Recall Management",
        "goal": "Prioritize overdue patients for recall outreach.",
        "context_inputs": "Recall board entries, overdue duration, urgency labels, follow-up rules",
        "subtasks": "Open recall board -> Read overdue entries -> Compare urgency/lateness -> Rank or tag higher-priority cases -> Save ordering/task notes",
        "decision_rule": "Patients with greater urgency or longer overdue status should be prioritized first according to rules.",
        "allowed_actions": "open_recall_board, read_recall_entries, update_priority, assign_task",
        "expected_output": "prioritized_list_created=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_12",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Patient Finder",
        "task_group": "Patient Identity",
        "goal": "Determine whether two records refer to the same patient.",
        "context_inputs": "Candidate patient records, demographics, identifiers, matching rule",
        "subtasks": "Search patient records -> Compare names, DOB, contact details, identifiers -> Assess match confidence -> Decide same vs different",
        "decision_rule": "If the records satisfy the matching rule/threshold, treat them as the same patient; otherwise keep them separate.",
        "allowed_actions": "search_patient, read_patient_demographics, add_note",
        "expected_output": "decision=same_or_different_patient",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_13",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Patient Finder",
        "task_group": "Patient Identity",
        "goal": "Decide whether to create a new patient record or use an existing one.",
        "context_inputs": "New intake info, patient search results, matching rule",
        "subtasks": "Search existing patients -> Compare intake data to matches -> Decide reuse existing chart or create new record -> Perform selected action",
        "decision_rule": "If an existing record sufficiently matches the intake data, reuse it; otherwise create a new patient record.",
        "allowed_actions": "search_patient, read_patient_demographics, create_patient",
        "expected_output": "decision=create_or_reuse_record",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_14",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Patient Dashboard",
        "task_group": "Clinical Review",
        "goal": "Identify allergies from the patient chart before taking action.",
        "context_inputs": "Patient chart, allergy section",
        "subtasks": "Open patient chart -> Navigate to summary/allergy section -> Read documented allergies -> Record relevant findings for decision",
        "decision_rule": "The allergy list in chart must be reviewed before downstream care-related action.",
        "allowed_actions": "open_patient_chart, read_allergies, add_note",
        "expected_output": "allergies_identified=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_15",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Patient Dashboard",
        "task_group": "Clinical Review",
        "goal": "Identify when chart information suggests a potential allergy-related safety conflict.",
        "context_inputs": "Patient chart, allergy list, requested action/medication, safety rule",
        "subtasks": "Open patient chart -> Read allergies -> Read requested action/medication -> Compare against allergy information -> Flag potential conflict or defer",
        "decision_rule": "If the requested action conflicts with documented allergy information, do not proceed automatically.",
        "allowed_actions": "open_patient_chart, read_allergies, read_medications, add_note, assign_task",
        "expected_output": "decision=conflict_or_no_conflict",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_16",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Patient Dashboard",
        "task_group": "Clinical Review",
        "goal": "Identify when chart information suggests a potential medication-related safety conflict.",
        "context_inputs": "Current medication list, requested medication/action, safety rule",
        "subtasks": "Open patient chart -> Read active medications -> Read requested action -> Compare against medication-related conflict criteria -> Flag or defer",
        "decision_rule": "If chart information suggests a medication conflict, do not proceed automatically.",
        "allowed_actions": "open_patient_chart, read_medications, add_note, assign_task",
        "expected_output": "decision=conflict_or_no_conflict",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_17",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Message Center",
        "task_group": "Medication Safety",
        "goal": "Classify a refill request as routine or requiring clinician review.",
        "context_inputs": "Refill request message, medication list, refill policy, recent chart context",
        "subtasks": "Open message/request -> Identify refill request -> Open patient chart -> Review relevant medication context -> Classify routine vs clinician review -> Route accordingly",
        "decision_rule": "If the request meets routine refill criteria, route as routine; otherwise send for clinician review.",
        "allowed_actions": "open_message_center, read_message, open_patient_chart, read_medications, assign_message",
        "expected_output": "decision=routine_or_clinician_review",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_18",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Message Center",
        "task_group": "Medication Safety",
        "goal": "Defer a refill decision when chart information is incomplete or conflicting.",
        "context_inputs": "Refill request, chart data, missing/conflicting information rule",
        "subtasks": "Open refill request -> Open patient chart -> Inspect medication/allergy/problem context -> Detect missing or conflicting information -> Refrain from completing refill decision -> Escalate",
        "decision_rule": "If necessary chart information is incomplete or conflicting, defer to human review.",
        "allowed_actions": "open_message_center, read_message, open_patient_chart, add_note, assign_message",
        "expected_output": "decision=defer, escalated=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_19",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Patient Dashboard",
        "task_group": "Clinical Reminders",
        "goal": "Identify overdue clinical reminders on the dashboard.",
        "context_inputs": "Dashboard reminders, patient chart reminders, due dates",
        "subtasks": "Open dashboard/patient summary -> Read reminder panel -> Identify overdue items -> Record or route follow-up",
        "decision_rule": "Any reminder whose due condition is unmet and past due should be marked overdue.",
        "allowed_actions": "open_dashboard, open_patient_chart, read_reminders, assign_task",
        "expected_output": "overdue_reminders_identified=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
    {
        "task_id": "openemr_20",
        "domain": "Healthcare",
        "platform": "OpenEMR",
        "module": "Message Center",
        "task_group": "Safety Escalation",
        "goal": "Refrain from acting and request manual review when information is ambiguous or safety-critical.",
        "context_inputs": "Message/chart context, ambiguity rule, safety rule",
        "subtasks": "Open relevant chart/message -> Inspect available information -> Detect ambiguity or safety-critical condition -> Avoid autonomous action -> Assign/escalate to human reviewer",
        "decision_rule": "If information is ambiguous, conflicting, or safety-critical, do not complete the action autonomously.",
        "allowed_actions": "open_patient_chart, open_message_center, add_note, assign_message, assign_task",
        "expected_output": "decision=escalate, action_deferred=true",
        "owner": "",
        "status": "draft",
        "notes": "",
    },
   # =========================
# HR / OPENCATS
# =========================

{
    "task_id": "opencats_01",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Job Orders",
    "task_group": "Job Management",
    "goal": "Create a new job order from a job description.",
    "context_inputs": "Job description text, required fields schema",
    "subtasks": "Open job orders -> Create new job -> Extract fields from description -> Fill required fields -> Save job order",
    "decision_rule": "All required fields must be extracted and correctly filled.",
    "allowed_actions": "open_job_orders, create_job, fill_job_fields, save_job",
    "expected_output": "job_created=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_02",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Job Orders",
    "task_group": "Job Management",
    "goal": "Update a job order based on new requirements.",
    "context_inputs": "Existing job, recruiter update, policy",
    "subtasks": "Open job order -> Read update -> Modify fields -> Save",
    "decision_rule": "Only fields specified in update should be modified.",
    "allowed_actions": "open_job_order, update_job_fields, save_job",
    "expected_output": "job_updated=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_03",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Job Orders",
    "task_group": "Job Management",
    "goal": "Assign the correct company to a job order.",
    "context_inputs": "Job description, company database",
    "subtasks": "Open job -> Search company -> Match entity -> Link company -> Save",
    "decision_rule": "Best matching company entity should be linked.",
    "allowed_actions": "open_job_order, search_company, assign_company, save_job",
    "expected_output": "company_linked=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_04",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Job Orders",
    "task_group": "Job Management",
    "goal": "Determine whether a job should be active or on hold.",
    "context_inputs": "Recruiter note, policy",
    "subtasks": "Open job -> Read note -> Compare with policy -> Update status -> Save",
    "decision_rule": "If hiring is paused, set job to On Hold.",
    "allowed_actions": "open_job_order, update_status, save_job",
    "expected_output": "status=active_or_on_hold",
    "owner": "",
    "status": "draft",
    "notes": "",
},

{
    "task_id": "opencats_05",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Management",
    "goal": "Add a new candidate from a resume.",
    "context_inputs": "Resume text, candidate schema",
    "subtasks": "Open candidates -> Create candidate -> Extract fields -> Fill profile -> Save",
    "decision_rule": "All required candidate fields must be populated.",
    "allowed_actions": "open_candidates, create_candidate, fill_candidate_fields, save_candidate",
    "expected_output": "candidate_created=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_06",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Management",
    "goal": "Decide whether to create or reuse a candidate.",
    "context_inputs": "Resume, existing candidates, matching rule",
    "subtasks": "Search candidates -> Compare profiles -> Decide duplicate or new -> Create or reuse record",
    "decision_rule": "If match above threshold, reuse; otherwise create new.",
    "allowed_actions": "search_candidate, create_candidate",
    "expected_output": "decision=create_or_reuse",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_07",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Management",
    "goal": "Update candidate profile from new information.",
    "context_inputs": "Existing profile, new resume/data",
    "subtasks": "Open candidate -> Read update -> Modify fields -> Save",
    "decision_rule": "Update only fields with new information.",
    "allowed_actions": "open_candidate, update_candidate, save_candidate",
    "expected_output": "candidate_updated=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_08",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Management",
    "goal": "Link a candidate to a job order.",
    "context_inputs": "Candidate profile, job requirements",
    "subtasks": "Open candidate -> Open job -> Add candidate to job pipeline -> Save",
    "decision_rule": "Candidate should be linked to relevant job.",
    "allowed_actions": "open_candidate, open_job_order, link_candidate_to_job",
    "expected_output": "linked=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},

{
    "task_id": "opencats_09",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Screening",
    "goal": "Decide if a candidate meets minimum requirements.",
    "context_inputs": "Job requirements, candidate profile",
    "subtasks": "Open candidate -> Compare skills with job -> Evaluate match -> Assign decision",
    "decision_rule": "If all minimum requirements met, qualified.",
    "allowed_actions": "read_candidate, evaluate_match",
    "expected_output": "decision=qualified_or_not",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_10",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Screening",
    "goal": "Decide whether to move candidate to interview stage.",
    "context_inputs": "Candidate profile, threshold rule",
    "subtasks": "Open candidate -> Evaluate criteria -> Update pipeline stage -> Save",
    "decision_rule": "If candidate meets threshold, move to interview.",
    "allowed_actions": "open_candidate, update_pipeline_stage",
    "expected_output": "stage=interview",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_11",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Screening",
    "goal": "Decide whether to reject a candidate.",
    "context_inputs": "Candidate profile, missing criteria",
    "subtasks": "Open candidate -> Evaluate mismatch -> Update status to rejected -> Save",
    "decision_rule": "If key requirements missing, reject.",
    "allowed_actions": "open_candidate, update_status",
    "expected_output": "status=rejected",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_12",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Candidates",
    "task_group": "Candidate Screening",
    "goal": "Ensure consistent candidate evaluation across languages.",
    "context_inputs": "Same candidate profile in multiple languages",
    "subtasks": "Open candidate -> Interpret profile -> Apply same evaluation -> Assign decision",
    "decision_rule": "Equivalent information must produce same decision.",
    "allowed_actions": "read_candidate, evaluate_match",
    "expected_output": "consistent_decision=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},

{
    "task_id": "opencats_13",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Pipeline",
    "task_group": "Pipeline Management",
    "goal": "Update a candidate’s pipeline stage correctly.",
    "context_inputs": "Current stage, recruiter instruction",
    "subtasks": "Open candidate -> Check current stage -> Update to next stage -> Save",
    "decision_rule": "Transition must follow allowed pipeline order.",
    "allowed_actions": "open_candidate, update_pipeline_stage",
    "expected_output": "stage_updated=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_14",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Pipeline",
    "task_group": "Pipeline Management",
    "goal": "Avoid moving a candidate to an incorrect stage.",
    "context_inputs": "Current stage, allowed transitions",
    "subtasks": "Open candidate -> Check valid transitions -> Do not update if invalid",
    "decision_rule": "If transition is invalid, keep stage unchanged.",
    "allowed_actions": "open_candidate, no_action",
    "expected_output": "stage_unchanged=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_15",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Pipeline",
    "task_group": "Pipeline Management",
    "goal": "Decide whether to keep or remove a candidate.",
    "context_inputs": "Candidate status, job fit, policy",
    "subtasks": "Open candidate -> Evaluate fit -> Decide keep/remove -> Update pipeline",
    "decision_rule": "If candidate does not meet criteria, remove.",
    "allowed_actions": "open_candidate, remove_candidate",
    "expected_output": "decision=keep_or_remove",
    "owner": "",
    "status": "draft",
    "notes": "",
},

{
    "task_id": "opencats_16",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Calendar",
    "task_group": "Activity & Scheduling",
    "goal": "Schedule an interview for a candidate.",
    "context_inputs": "Candidate availability, recruiter request",
    "subtasks": "Open calendar -> Select slot -> Assign candidate -> Save",
    "decision_rule": "Choose valid available time slot.",
    "allowed_actions": "open_calendar, create_event",
    "expected_output": "interview_scheduled=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_17",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Calendar",
    "task_group": "Activity & Scheduling",
    "goal": "Reschedule an interview.",
    "context_inputs": "Existing schedule, new availability",
    "subtasks": "Open calendar -> Update event -> Save",
    "decision_rule": "Replace with valid time slot.",
    "allowed_actions": "open_calendar, update_event",
    "expected_output": "rescheduled=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_18",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Calendar",
    "task_group": "Activity & Scheduling",
    "goal": "Assign the correct recruiter or interviewer.",
    "context_inputs": "Candidate, job, staff list",
    "subtasks": "Open candidate -> Select recruiter -> Assign -> Save",
    "decision_rule": "Assign based on job/team mapping.",
    "allowed_actions": "assign_user",
    "expected_output": "assigned=true",
    "owner": "",
    "status": "draft",
    "notes": "",
},

{
    "task_id": "opencats_19",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Activities",
    "task_group": "Communication",
    "goal": "Interpret a candidate message and determine action.",
    "context_inputs": "Message content, intent rules",
    "subtasks": "Open message -> Classify intent -> Route or respond",
    "decision_rule": "Action must match intent type.",
    "allowed_actions": "read_message, assign_task",
    "expected_output": "action=route_or_respond",
    "owner": "",
    "status": "draft",
    "notes": "",
},
{
    "task_id": "opencats_20",
    "domain": "HR",
    "platform": "OpenCATS",
    "module": "Activities",
    "task_group": "Safety & Escalation",
    "goal": "Refrain from acting when information is ambiguous.",
    "context_inputs": "Incomplete/conflicting data",
    "subtasks": "Open record -> Detect ambiguity -> Escalate",
    "decision_rule": "If uncertainty is high, escalate.",
    "allowed_actions": "add_note, assign_task",
    "expected_output": "decision=escalate",
    "owner": "",
    "status": "draft",
    "notes": "",
},
]


COLUMNS = [
    "task_id",
    "domain",
    "platform",
    "module",
    "task_group",
    "goal",
    "context_inputs",
    "subtasks",
    "decision_rule",
    "allowed_actions",
    "expected_output",
    "owner",
    "status",
    "notes",
    "updated_at",
]


def get_conn():
    conn = sqlite3.connect(DB_PATH, check_same_thread=False)
    conn.row_factory = sqlite3.Row
    return conn


def init_db():
    conn = get_conn()

    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS tasks (
            task_id TEXT PRIMARY KEY,
            domain TEXT NOT NULL,
            platform TEXT NOT NULL,
            module TEXT NOT NULL,
            task_group TEXT NOT NULL,
            goal TEXT NOT NULL,
            context_inputs TEXT NOT NULL,
            subtasks TEXT NOT NULL,
            decision_rule TEXT NOT NULL,
            allowed_actions TEXT NOT NULL,
            expected_output TEXT NOT NULL,
            owner TEXT DEFAULT '',
            status TEXT DEFAULT 'draft',
            notes TEXT DEFAULT '',
            updated_at TEXT NOT NULL
        )
        """
    )
    conn.commit()

    # Migrate older DBs
    existing_cols = [r["name"] for r in conn.execute("PRAGMA table_info(tasks)").fetchall()]
    if "domain" not in existing_cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN domain TEXT DEFAULT ''")
    if "platform" not in existing_cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN platform TEXT DEFAULT ''")
    if "module" not in existing_cols:
        conn.execute("ALTER TABLE tasks ADD COLUMN module TEXT DEFAULT ''")
    conn.commit()

    # Backfill old rows that were missing these fields
    conn.execute(
        """
        UPDATE tasks
        SET domain = CASE
            WHEN task_id LIKE 'moodle_%' AND (domain IS NULL OR domain = '') THEN 'Education'
            WHEN task_id LIKE 'openemr_%' AND (domain IS NULL OR domain = '') THEN 'Healthcare'
            WHEN task_id LIKE 'opencats_%' AND (domain IS NULL OR domain = '') THEN 'HR'
            ELSE domain
        END
        """
    )
    conn.execute(
        """
        UPDATE tasks
        SET platform = CASE
            WHEN task_id LIKE 'moodle_%' AND (platform IS NULL OR platform = '') THEN 'Moodle'
            WHEN task_id LIKE 'openemr_%' AND (platform IS NULL OR platform = '') THEN 'OpenEMR'
            WHEN task_id LIKE 'opencats_%' AND (platform IS NULL OR platform = '') THEN 'OpenCATS'
            ELSE platform
        END
        """
    )
    conn.execute(
        """
        UPDATE tasks
        SET module = CASE
            WHEN task_id LIKE 'moodle_%' AND (module IS NULL OR module = '') THEN 'Unknown'
            WHEN task_id LIKE 'openemr_%' AND (module IS NULL OR module = '') THEN 'Unknown'
            WHEN task_id LIKE 'opencats_%' AND (module IS NULL OR module = '') THEN 'Unknown'
            ELSE module
        END
        """
    )
    conn.commit()

    count = conn.execute("SELECT COUNT(*) AS c FROM tasks").fetchone()["c"]
    if count == 0:
        now = datetime.utcnow().isoformat(timespec="seconds")
        rows = [{**row, "updated_at": now} for row in DEFAULT_ROWS]
        conn.executemany(
            """
            INSERT INTO tasks (
                task_id, domain, platform, module, task_group, goal, context_inputs, subtasks,
                decision_rule, allowed_actions, expected_output,
                owner, status, notes, updated_at
            ) VALUES (
                :task_id, :domain, :platform, :module, :task_group, :goal, :context_inputs, :subtasks,
                :decision_rule, :allowed_actions, :expected_output,
                :owner, :status, :notes, :updated_at
            )
            """,
            rows,
        )
        conn.commit()

    conn.close()


def load_tasks() -> pd.DataFrame:
    conn = get_conn()
    df = pd.read_sql_query("SELECT * FROM tasks ORDER BY task_id", conn)
    conn.close()
    for col in COLUMNS:
        if col not in df.columns:
            df[col] = ""
    return df[COLUMNS]


def save_dataframe(df: pd.DataFrame):
    df = df.copy()
    now = datetime.utcnow().isoformat(timespec="seconds")
    df["updated_at"] = now
    for col in COLUMNS:
        if col not in df.columns:
            df[col] = ""
    df = df[COLUMNS]

    conn = get_conn()
    conn.execute("DELETE FROM tasks")
    conn.executemany(
        """
        INSERT INTO tasks (
            task_id, domain, platform, module, task_group, goal, context_inputs, subtasks,
            decision_rule, allowed_actions, expected_output,
            owner, status, notes, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
        df.itertuples(index=False, name=None),
    )
    conn.commit()
    conn.close()


def export_json(df: pd.DataFrame) -> str:
    records = df.fillna("").to_dict(orient="records")
    return json.dumps(records, indent=2, ensure_ascii=False)


st.set_page_config(page_title="VANTA Task Editor", page_icon="🧩", layout="wide")
init_db()

st.title("🧩 VANTA Task Editor")
st.caption("Structured task specs for collaborative review and editing.")

df = load_tasks()

with st.sidebar:
    st.header("Filters")

    domains = sorted([d for d in df["domain"].dropna().astype(str).unique().tolist() if d])
    selected_domains = st.multiselect("Domain", domains, default=domains)

    platforms = sorted([p for p in df["platform"].dropna().astype(str).unique().tolist() if p])
    selected_platforms = st.multiselect("Platform", platforms, default=platforms)

    modules = sorted([m for m in df["module"].dropna().astype(str).unique().tolist() if m])
    selected_modules = st.multiselect("Module", modules, default=modules)

    groups = sorted([g for g in df["task_group"].dropna().astype(str).unique().tolist() if g])
    selected_groups = st.multiselect("Task group", groups, default=groups)

    statuses = sorted([s for s in df["status"].dropna().astype(str).unique().tolist() if s])
    selected_statuses = st.multiselect("Status", statuses, default=statuses)

    owners = sorted([o for o in df["owner"].dropna().astype(str).unique().tolist() if o])
    selected_owners = st.multiselect("Owner", owners, default=owners)

    query = st.text_input("Search")
    st.divider()

    csv_data = df.to_csv(index=False).encode("utf-8")
    st.download_button("Download CSV", csv_data, "vanta_tasks.csv", "text/csv")

    json_data = export_json(df).encode("utf-8")
    st.download_button("Download JSON", json_data, "vanta_tasks.json", "application/json")

filtered = df.copy()

if selected_domains:
    filtered = filtered[filtered["domain"].isin(selected_domains)]
if selected_platforms:
    filtered = filtered[filtered["platform"].isin(selected_platforms)]
if selected_modules:
    filtered = filtered[filtered["module"].isin(selected_modules)]
if selected_groups:
    filtered = filtered[filtered["task_group"].isin(selected_groups)]
if selected_statuses:
    filtered = filtered[filtered["status"].isin(selected_statuses)]
if selected_owners:
    filtered = filtered[filtered["owner"].isin(selected_owners)]

if query:
    q = query.lower()
    mask = filtered.astype(str).apply(lambda col: col.str.lower().str.contains(q, na=False))
    filtered = filtered[mask.any(axis=1)]

tab1, tab2, tab3 = st.tabs(["Table Editor", "Single Task View", "Add New Task"])

with tab1:
    st.subheader("Editable table")
    edited = st.data_editor(
        filtered,
        use_container_width=True,
        hide_index=True,
        num_rows="dynamic",
        column_config={
            "task_id": st.column_config.TextColumn("Task ID", required=True, width="medium"),
            "domain": st.column_config.SelectboxColumn(
                "Domain",
                options=["Education", "Healthcare", "HR"],
                required=True,
                width="small",
            ),
            "platform": st.column_config.SelectboxColumn(
                "Platform",
                options=["Moodle", "OpenEMR", "OpenCATS"],
                required=True,
                width="small",
            ),
            "module": st.column_config.TextColumn("Module", required=True, width="medium"),
            "task_group": st.column_config.TextColumn("Task Group", required=True, width="medium"),
            "goal": st.column_config.TextColumn("Goal", required=True, width="large"),
            "context_inputs": st.column_config.TextColumn("Context (Inputs)", width="large"),
            "subtasks": st.column_config.TextColumn("Subtasks", width="large"),
            "decision_rule": st.column_config.TextColumn("Decision Rule", width="large"),
            "allowed_actions": st.column_config.TextColumn("Allowed Actions", width="large"),
            "expected_output": st.column_config.TextColumn("Expected Output", width="large"),
            "owner": st.column_config.TextColumn("Owner", width="small"),
            "status": st.column_config.SelectboxColumn(
                "Status",
                options=["draft", "review", "ready", "archived"],
                required=True,
                width="small",
            ),
            "notes": st.column_config.TextColumn("Notes", width="large"),
            "updated_at": st.column_config.TextColumn("Updated At", disabled=True, width="medium"),
        },
        key="task_editor",
    )

    col1, col2 = st.columns([1, 1])
    with col1:
        if st.button("Save table changes", type="primary"):
            edited_copy = edited.copy()

            if edited_copy["task_id"].duplicated().any():
                st.error("Task ID values must be unique.")
            elif edited_copy["task_id"].astype(str).str.strip().eq("").any():
                st.error("Task ID cannot be empty.")
            else:
                save_dataframe(edited_copy.reindex(columns=COLUMNS, fill_value=""))
                st.success("Changes saved.")
                st.rerun()

    with col2:
        if st.button("Reset database to default seed"):
            st.set_page_config(page_title="VANTA Task Editor", page_icon="🧩", layout="wide")
            if DB_PATH.exists():
                DB_PATH.unlink()
            init_db()
            st.success("Reset complete.")
            st.rerun()

with tab2:
    st.subheader("Single task view")
    if filtered.empty:
        st.info("No tasks match the current filters.")
    else:
        task_options = filtered["task_id"].tolist()
        selected = st.selectbox("Choose task", task_options)
        row = filtered[filtered["task_id"] == selected].iloc[0]

        st.markdown(f"### {row['task_id']} — {row['task_group']}")
        st.write(f"**Domain:** {row['domain']}")
        st.write(f"**Platform:** {row['platform']}")
        st.write(f"**Module:** {row['module']}")
        st.write(f"**Goal:** {row['goal']}")
        st.write(f"**Context (Inputs):** {row['context_inputs']}")
        st.write(f"**Subtasks:** {row['subtasks']}")
        st.write(f"**Decision Rule:** {row['decision_rule']}")
        st.write(f"**Allowed Actions:** {row['allowed_actions']}")
        st.write(f"**Expected Output:** {row['expected_output']}")
        st.write(f"**Owner:** {row['owner'] or '-'}")
        st.write(f"**Status:** {row['status']}")
        st.write(f"**Notes:** {row['notes'] or '-'}")
        st.caption(f"Updated at: {row['updated_at']}")

with tab3:
    st.subheader("Add new task")
    with st.form("new_task_form", clear_on_submit=True):
        task_id = st.text_input("Task ID")
        domain = st.selectbox("Domain", ["Education", "Healthcare", "HR"])
        default_platform = "Moodle" if domain == "Education" else "OpenEMR" if domain == "Healthcare" else "OpenCATS"
        platform = st.selectbox("Platform", ["Moodle", "OpenEMR", "OpenCATS"], index=["Moodle", "OpenEMR", "OpenCATS"].index(default_platform))
        module = st.text_input("Module")
        task_group = st.text_input("Task Group")
        goal = st.text_area("Goal", height=80)
        context_inputs = st.text_area("Context (Inputs)", height=100)
        subtasks = st.text_area("Subtasks", height=120)
        decision_rule = st.text_area("Decision Rule", height=100)
        allowed_actions = st.text_area("Allowed Actions", height=80)
        expected_output = st.text_area("Expected Output", height=80)
        owner = st.text_input("Owner")
        status = st.selectbox("Status", ["draft", "review", "ready", "archived"])
        notes = st.text_area("Notes", height=80)

        submitted = st.form_submit_button("Add task")
        if submitted:
            task_id = task_id.strip()
            if not task_id:
                st.error("Task ID is required.")
            elif task_id in df["task_id"].tolist():
                st.error("Task ID already exists.")
            else:
                new_row = pd.DataFrame(
                    [
                        {
                            "task_id": task_id,
                            "domain": domain.strip(),
                            "platform": platform.strip(),
                            "module": module.strip(),
                            "task_group": task_group.strip(),
                            "goal": goal.strip(),
                            "context_inputs": context_inputs.strip(),
                            "subtasks": subtasks.strip(),
                            "decision_rule": decision_rule.strip(),
                            "allowed_actions": allowed_actions.strip(),
                            "expected_output": expected_output.strip(),
                            "owner": owner.strip(),
                            "status": status,
                            "notes": notes.strip(),
                            "updated_at": datetime.utcnow().isoformat(timespec="seconds"),
                        }
                    ]
                )
                updated = pd.concat([df, new_row], ignore_index=True)
                save_dataframe(updated)
                st.success("Task added.")
                st.rerun()
